...
- Making the plugin easier to install on SonarQube 9.x+ deployments (including Helm/Kubernetes), improving rule visibility/loading reliability, improving issue locations, and tightening dependencies.
- Security hardening for safer use in CI/PR analysis environments.
- Adds a project naming convention rule for Mule 4 and the supporting `project` scope in the rule engine.
- Updates the Mule 4 project naming convention rule to validate `mule-artifact/project` via XPath (instead of SonarQube project metadata), ensuring enforcement is driven by the Mule artifact descriptor committed to source control.
- Expands the built-in Mule 4 namespace mappings to cover more commonly used modules/connectors, improving rule and metric evaluation for projects using those namespaces.
- Introduces **v1.1 rules** backed by the standard `javax.xml` (JAXP) XPath engine, enabling **node-based rules** with accurate issue locations and multiple issues per rule per file.

### Compatibility

- **Minimum SonarQube version**: **9.9 LTS** (the plugin is now built against `sonar-plugin-api` 9.9).
...
- **#59 – jackson-databind 2.12.6.1 vulnerability**
  - Updated `jackson-databind` dependency to `2.21.1`.
...
- **#30 – Minimum Sonarqube version not documented**
  - Documented **minimum SonarQube version 9.9 LTS** (based on `sonar-plugin-api` 9.9).
...
- **Dependencies refreshed**
  - Updated core libraries used by rule evaluation and XML handling (including `sonar-xml-parsing`, `jaxen`, JAXB runtime, and `jackson-databind`).
  - Updated build tooling (`sonar-packaging-maven-plugin`) and aligned test dependencies (JUnit).
  - Marked `slf4j-api` as `provided` to avoid shipping an SLF4J API copy into SonarQube.
...
- **Rules packaging**: 
  - `rules-3.xml` and `rules-4.xml` are now embedded in the plugin JAR by default. If you want to customize rules without rebuilding, place rule files at `extensions/plugins/rules-3.xml` and/or `extensions/plugins/rules-4.xml` on the SonarQube server.
- **Scanning property**: 
